...
/* ============================================================
   PROFILE CARD - RENDERED UI STYLE
   ============================================================ */

.user-profile-card {
    background: linear-gradient(135deg, rgba(28,32,68,0.92), rgba(12,16,42,0.96));
    border: 1px solid rgba(130,115,255,0.40);
    border-radius: 16px;
    padding: 14px;
    margin-bottom: 18px;
    display: flex;
...
.quantum-tutor-title {
    ...
    text-shadow: 0 0 30px rgba(103,191,255,0.25);
}
...
if not st.session_state.logged_in:
    st.html(
        ...
        <div class="login-container">
            ...
            <p style="color: #8c98ba; font-size: 13px; margin-top: 6px;">
...
with st.sidebar:
    ...
    st.markdown(
        "<div style='margin: 10px 0; height: 1px; background: rgba(255,255,255,0.08);'></div>",
        unsafe_allow_html=True
    )
    ...
    for chat_name in list(st.session_state.chats.keys()):
        ...
        chat_col, menu_col = st.columns([0.84, 0.16])
        ...
        with menu_col:
            with st.popover("⋮", help=f"Manage {chat_name}"):

                st.markdown("**Manage Chat**")

                if st.button(
                    "✏️ Rename Chat",
                    key=f"rename_btn_{chat_name}",
                    use_container_width=True
                ):
    ...
    if st.session_state.rename_chat:
        ...
        rename_target = st.session_state.rename_chat
        ...
        with rename_col1:
            if st.button(
                ...
                cleaned_name = new_chat_name.strip()

                if not cleaned_name:
                    st.warning("Chat name cannot be empty.")

                elif (
                    cleaned_name != rename_target
                    and cleaned_name in st.session_state.chats
                ):
                    st.warning("A chat with this name already exists.")

                else:
                    st.session_state.chats[cleaned_name] = (
                        st.session_state.chats.pop(rename_target)
                    )

                    if st.session_state.current_chat == rename_target:
                        st.session_state.current_chat = cleaned_name
    ...
    if st.session_state.delete_chat:
        ...
        delete_target = st.session_state.delete_chat

        st.warning(
            f"Delete **{delete_target}**? This cannot be undone."
        )
        ...
        with delete_col1:
            if st.button(
                ...
                del st.session_state.chats[delete_target]
                ...
                elif st.session_state.current_chat == delete_target:
    ...
    if st.session_state.clear_chat:
        ...
        clear_target = st.session_state.clear_chat

        st.warning(
            f"Clear all messages from **{clear_target}**?"
        )
        ...
        with clear_col1:
            if st.button(
                ...
                st.session_state.chats[clear_target] = []
                st.session_state.clear_chat = None
                st.rerun()
    ...
    st.markdown(
        "<div style='margin: 14px 0; height: 1px; background: rgba(255,255,255,0.08);'></div>",
        unsafe_allow_html=True
    )
...
if st.session_state.learning_mode:
    ...
    lessons = {
        "Quantum Computing Basics": {
            "intro": "Quantum computing uses quantum-mechanical effects to process information. Instead of classical bits, quantum computers use qubits.",
            "points": [
                "A classical bit is either 0 or 1.",
            ...
            "example": "A classical bit is like a switch that is OFF or ON. A qubit is described using amplitudes for |0⟩ and |1⟩."
        },
        "Qubits & Superposition": {
            "intro": "A qubit is the basic unit of quantum information. Superposition allows a qubit to be represented as a combination of |0⟩ and |1⟩.",
            "points": [
            ...
            "example": "For (|0⟩ + |1⟩)/√2, an ideal computational-basis measurement gives 0 or 1 with equal probability."
        },
        "Entanglement": {
            "intro": "Entanglement is a quantum correlation where the joint state of multiple qubits cannot be described as independent states of each qubit.",
            "points": [
                "Entangled qubits share a joint quantum state.",
            ...
            "example": "The Bell state (|00⟩ + |11⟩)/√2 produces correlated measurement outcomes."
        },
        "Quantum Gates": {
            "intro": "Quantum gates are operations that change quantum states.",
            "points": [
            ...
            "example": "Starting with |0⟩ and applying H produces (|0⟩ + |1⟩)/√2."
        },
        "Quantum Circuits": {
            "intro": "A quantum circuit is a sequence of quantum gates applied to qubits, usually followed by measurement.",
            "points": [
            ...
            "example": "A simple circuit can prepare |0⟩, apply H, and then measure the qubit."
        },
        "Measurement": {
            "intro": "Measurement extracts classical information from a quantum state and generally changes that state.",
            "points": [
            ...
            "example": "For (|0⟩ + |1⟩)/√2, ideal measurements produce approximately half 0s and half 1s."
        },
        "Grover's Algorithm": {
            "intro": "Grover's algorithm gives a quadratic speedup for searching an unstructured space in the standard quantum query model.",
            "points": [
            ...
            "example": "A classical black-box search can require O(N) queries, while Grover's algorithm requires O(√N) queries."
        },
        "Shor's Algorithm": {
            "intro": "Shor's algorithm is a quantum algorithm for integer factoring using efficient quantum period finding.",
            "points": [
            ...
            "example": "Shor's algorithm converts a factoring problem into a period-finding problem that can be processed using quantum interference."
        }
    }
    ...
    st.markdown(
        """
        <div class="learning-panel">
            <div class="learning-title">🧠 QUANTUM COMPUTING LEARNING MODE</div>
            <div class="learning-subtitle">
                Learn quantum computing step by step with explanations,
    ...
    selected_topic = st.selectbox(
        ...
        topics,
        index=topics.index(st.session_state.learning_topic)
        if st.session_state.learning_topic in topics else 0
    )
    ...
    completed = sum(
        ...
        if st.session_state.learning_progress.get(topic, False)
    )

    st.progress(
        completed / len(topics),
        text=f"Learning progress: {completed}/{len(topics)} topics completed"
    )
